...
hist(dat$Biomass, col="palevioletred1", main= "Simple Histogram of Biomass", xlab = 'Biomass')   # makes simple HISTOGRAM of Biomass (base R)
...
ggplot(dat,aes(x=Biomass))+  # "tells ggplot" what is the dataset and the variables to be plotted
  ...
  ggtitle("Simple Histogram of Biomass") +    # Adds a title to our plot, by default it is left centered
  theme_minimal()                             # sets the way the plot looks

# BOX PLOT #                             

boxplot(Biomass~Species,dat, col=c(2:6), main= "Species' Biomass" )            # makes simple BOX PLOT of Biomass ~ Species (base R)
                                                    # note that now, I used a vector c(2:6) to tell R, the colors. 

# EXERCISE 1: Try changing the numbers in the color vector to see what happens. 
# EXERCISE 2: Make a simple boxplot using ggplot. Add a title and choose a theme. 
# EXERCISE 2.CHALLANGE: center the title. (I would google "how to center title in ggplot)


ggplot(dat,aes(x=Species,y=Biomass))+         # tells ggplot what is the dataframe and the variables to be plotted
  geom_boxplot(colour = "black", fill = "limegreen", width = .5) +         # tells ggplot what kind of plot we want (box plot), the color and widteh of the boxes
  ggtitle("Simple Boxplot of Biomass") +    # Adds a title to our plot
  theme_bw() +
  easy_center_title()
...
ggplot(dat_sum %>% 
  ...
  geom_bar(colour = "black", fill = "cornsilk3", stat='identity') + 
  ggtitle("Corn yield over the years") +    # Adds a title to our plot
  easy_center_title()

# EXERCISE 3: Why do we need "stat = 'identity'"? Try googling the following to find out: "geom_bar(stat='identity')"
# EXERCISE 4: Make the same bar plot using base R 

barplot(yield~Year,dat_sum %>%          
          filter(Species=='Corn' & Treatment=='T1' & Fraction=='SEED'), 
        main="Corn yield over the years", col="cornsilk3") # makes simple BAR PLOT of Yield ~ Year but only for Corn, in Treatment 1, and fraction = seed (base R)


# SCATTER PLOT #

# Exercise 5: Make a scatter plot of Biomass~ Year. (hint, the function for scatter plot is plot()). Dont forget to add a title.
# Exercise 5.CHALLENGE: change the symbol of the point so it is a squares

plot(Biomass~Year,dat, main="Biomass vs. years", pch=22)
legend(x='topright', legend=c("biomass"), pch=22) # adds a legend on the top right corner of our plot. 

 
ggplot(dat,aes(x=Year,y=Biomass))+
  geom_point() +
  ggtitle("Corn yield by year") +    # Adds a title to our plot
  easy_center_title()
  

########### IMPROVING OUR PLOTS IN GGPLOT ########################

# Note that until now we have been only making very simple plots.
...
# EXERCISE 6: Try changing up values in the code and see how that changes your plot. 
...
plot(my_spdf, col="#f2f2f2", bg="skyblue", lwd=0.25, border=1 )
...
# EXERCISE 7: Plot only the country and city you were born at. (hint: use filter and search for the coordinates of the city online)

cities <- data.frame(ID = c("Sao Paulo"),
                     x = c(-23.533773),
                     y = c(-46.625290))   # creates a data frame for the cities we visited, using latitude and longitude data

ggplot() +
  geom_polygon(data = spdf_fortified%>%filter(id=='Brazil'), aes( x = long, y = lat, group = group), fill="limegreen", color="white") +
  geom_point(data = cities, aes(x = y, y = x),col="yellow", size=1) + # add the cities
  theme_void()
...
p<-ggplot(dat_sum%>%
            filter(Fraction=='SEED' & Species == 'Wheat'), aes(x=Treatment, y=yield, fill=Treatment)) + 
...
mcyst<-read.csv("./Gull_Lake_mcyst.csv", header=T, skip=26)
mcyst<-mcyst[-1,]
...
mcyst$biomass<-as.numeric(mcyst$biomass_ug_l)

hist(as.numeric(mcyst$biomass)) # looks skeweed so I will use the log transformed variable
hist(as.numeric(mcyst$microcystin_ug_l))

model<-lm(log(as.numeric(microcystin_ug_l))~log_biomass+year, data=mcyst)
summary(model)

ggplot(data=mcyst, aes(x=log_biomass, y=log(as.numeric(microcystin_ug_l))))+
  geom_point(aes(color=as.factor(year))) +
  geom_smooth(method='lm') +
  ggtitle("Microcystin vs. Biomass") + 
  xlab("Microcystis biomass (log-ug/l)") + ylab("Microcystin concentration (log-ug/l)")+
