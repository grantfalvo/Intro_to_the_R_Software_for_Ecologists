...
grow_plant=function(temperature,sunlight){
  new_growth=temperature*2+sunlight*4
  return(new_growth)
}
...
weather_dat=data.frame(month=1:12,
                       temperature=c(0,0,0,5,5,10,10,5,5,0,0,0),
                       sunlight=c(1,2,3,4,5,6,6,5,4,3,2,1),
                       plant_growth=NA,
                       plant_height=NA)
...
weather_dat$plant_growth=grow_plant(weather_dat$temperature,
                                    weather_dat$sunlight)
weather_dat$plant_height=cumsum(weather_dat$plant_growth)

### plot your plant's growth over time

plot(plant_height~month,weather_dat,type='o')
